...
% Maneuver parameters
DeltaTheta = 5;                         % Flying angle change per incremental step (deg/s)
...
while y(n) > 0  && Mission_Success == false      % Run until rocket hits the ground or mission completed
  ...
  % Seperation logic
    ...
    if method == 1
      ...
      x(n) = (Delta^2) / Mass(n) * ( Thrust(n)*cosd(Theta(n-1)) ) + 2*x(n-1) - x(n-2);
      y(n) = (Delta^2) / Mass(n) * ( Thrust(n)*sind(Theta(n-1)) - Mass(n)* Gravity ) + 2*y(n-1) - y(n-2);
