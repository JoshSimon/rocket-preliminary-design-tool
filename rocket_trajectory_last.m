...
% Maneuver parameters
DeltaTheta = 0.1;              % Flying angle (deg/s)
...
while y(n) > 0  && Mission_Success == false      % Run until rocket hits the ground or mission completed
  ...
  if Mass(n) <= (Mass_Payload + Mass_Motor_And_Structure_Two) && Seperation_Two == false % seperation condition second stage
    ...
    if method == 1
      ...
      x(n) = (t(n)^2/ Mass(n)) * (    Thrust(n)*cosd(Theta(n-1))   ) + 2*x(n-1) - x(n-2);
      y(n) = (t(n)^2/ Mass(n)) * (    Thrust(n)*sind(Theta(n-1)) - Mass(n)* Gravity ) + 2*y(n-1) - y(n-2);
            
            disp('Thrust');disp(Thrust(n)*sind(Theta(n-1))- Mass(n)* Gravity);  
            disp('Anteil');disp( 2*y(n-1) - y(n-2));
